...
const RecommendUserService = class RecommendUserService {
    ...
    */
    constructor(data, userId, n) {
        ...
        // 记录除用户自身外，其他所有用户的ID
        this.userArray = []
        // 作为中间值，记录当前正在进行的其他用户的动态列表
        this.userGoodsTemp = []
        // 记录用户最终相似度列表
        this.similarityList = []
        // 记录用户相似度前n个用户中所有购买的动态与本用户不重复的动态
        this.targetGoods = []
        // 记录用户对筛选出的各动态感兴趣程度
    ...
    */
    start() {
        ...
        // 计算目标动态
        this.getTargetGoods()
        // 此时目标动态已存在this.targetGoods中, 然后去重
    ...
    */
    getTargetGoods() {
        ...
        // 截至目前，已获取用户相似度的逆序排序数组，以下为获取前n个相似用户购买的所有动态中本用户没买过的
...
module.exports = { RecommendUserService }